...
resource "aws_s3_bucket" "terraform_state" {
  ...
  tags = {
    Name        = "${var.project_name}-${data.aws_caller_identity.current.account_id}-terraform-state"
...
resource "aws_dynamodb_table" "terraform_state_lock" {
  name         = "${var.project_name}-${data.aws_caller_identity.current.account_id}-terraform-state-lock"
